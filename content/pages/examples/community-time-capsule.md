...
- **Glyph** — the kind of memory. {{GLYPH_COUNT}} values, 0 to {{MAX_GLYPH}}: 0 a first, 1 a place, 2 a person joining, 3 a person leaving, 4 a performance, 5 a mistake, 6 a joke, 7 a loss.
- **Palette** — one of {{PALETTE_COUNT}} colours, 0 to {{MAX_PALETTE}}. Give each year its own colour and the finished capsule reads as eight bands.
- **Motion** — {{MOTION_COUNT}} values, 0 to {{MAX_MOTION}}: quiet, sudden, slow, ongoing.
- **Magnitude** — one byte. How much it mattered, judged by the keeper. Say plainly in the invitation that this is one person's judgement, so nobody mistakes it for a fact.
...
Eight closes, 2018 first. Each keeper reads their year aloud as it goes. A close carries one reason byte — agree it in advance, for example 0 kept, 1 hard year, 2 the year we nearly stopped — and it is recorded permanently with that path.
